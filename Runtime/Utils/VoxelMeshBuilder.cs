...
namespace Voxelization.Utils
{
    ...
    {
        /// <summary>
        /// voxelVertices[0] - right face
        /// voxelVertices[1] - left face
        /// voxelVertices[2] - up face
        /// voxelVertices[3] - down face
        /// voxelVertices[4] - front face
        /// voxelVertices[5] - back face
        /// </summary>
        ...
        private static readonly int[,] voxelFaceIndices = {
            { 6, 2, 1, 5 }, { 7, 3, 0, 4 },
            { 4, 2, 3, 5 }, { 7, 1, 0, 6 },
            { 4, 6, 7, 5 }, { 3, 1, 0, 2 }
        };

        private static readonly int[,] voxelIndicesOffset = {
            { 2, 1, 0, 1, 3, 0 }, { 0, 1, 2, 0, 3, 1 },
        ...
        private static MeshData BuildVoxelMesh(VoxelOctree tree, float sizeScale, Matrix4x4[] bindPoses = null)
        {
            ...
            var adjMatrix = VoxelOctree.GetAdjacencyMatrix(tree.Voxels, minVoxelBox, maxVoxelBox, tree.VoxelSize, out voxelsNum);
        ...
        private static void PlaceSimplifiedVoxel(OctreeNode voxel, Vector3Int voxelsNum, byte[,,] voxelAdjMatrix, VoxelOctree tree, MeshData meshData, float sizeScale, out bool[] placedFaces)
        {
            ...
            var positionInt = MathUtils.TranslatePositionToGrid(minVoxelBox, nodePos, scale);
            int x = positionInt.x;
            int y = positionInt.y;
            int z = positionInt.z;
        ...
        private static void PlaceVoxelFace(Vector3 pos, float scale, MeshData meshData, int submesh, int face)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                meshData.Vertices.Add(pos + voxelVertices[voxelFaceIndices[face, i]] * scale);
            }

            for (int i = 0; i < 6; i++)
            {
                meshData.Indices[submesh].Add(count + voxelIndicesOffset[face, i]);
            }
        }
    }
}
