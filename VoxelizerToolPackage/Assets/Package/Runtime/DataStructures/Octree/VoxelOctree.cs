    ...
    public class VoxelOctree
    {
        ...
        public VoxelOctree(float voxelSize, IOctreeProcessor octreeProcessor, params Mesh[] sharedMeshes)
        {
            ...
            for (int i = 0; i < objectsNumber; i++)
            {
                ...
                min = MathUtils.Min(mesh.bounds.min, min);
                max = MathUtils.Max(mesh.bounds.max, max);
            }
        ...
        public static byte[,,] BuildAdjMatrix(List<OctreeNode> nodes, Vector3 minVoxelBox, Vector3 maxVoxelBox, float voxelSize, out Vector3Int voxelsNum)
        {
            ...
            foreach (var node in nodes)
            {
                ...
                int x = MathUtils.TranslatePosition(minVoxelBox.x, pos.x, voxelSize);
                int y = MathUtils.TranslatePosition(minVoxelBox.y, pos.y, voxelSize);
                int z = MathUtils.TranslatePosition(minVoxelBox.z, pos.z, voxelSize);
                voxels[x, y, z] = 1;
            }
        ...
        private void BuildNode(OctreeNode node, List<int> triangles)
        {
            ...
            if (nextDepth >= maxTreeDepth)
            {
                octreeProcessor.CallUpdateBounds(() => {
                    ...
                    {
                        currentParent.FilledMin = MathUtils.Min(prevNode.FilledMin, currentParent.FilledMin);
                        currentParent.FilledMax = MathUtils.Max(prevNode.FilledMax, currentParent.FilledMax);
        ...
        private void ProcessLeaf(OctreeNode parent, OctreeNode child, int childTriangle)
        {
            ...
            parent.FilledMin = MathUtils.Min(child.AABB.Min, parent.FilledMin);
            parent.FilledMax = MathUtils.Max(child.AABB.Max, parent.FilledMax);
            ...
            octreeProcessor.CallUpdateGlobalBounds(() => {
                Bounds[0] = MathUtils.Min(parent.FilledMin, Bounds[0]);
                Bounds[1] = MathUtils.Max(parent.FilledMax, Bounds[1]);
            });
        }
