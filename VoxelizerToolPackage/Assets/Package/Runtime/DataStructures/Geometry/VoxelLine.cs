...
namespace Voxelization.DataStructures.Geometry
{
    public struct VoxelLine
    {
        ...
        {
            if (Axis == 1 && (Hit.z > B1.z || MathUtils.IsEquals(Hit.z, B1.z, Mathf.Pow(10, -4))) &&
                             (Hit.z < B2.z || MathUtils.IsEquals(Hit.z, B2.z, Mathf.Pow(10, -4))) &&
                             (Hit.y > B1.y || MathUtils.IsEquals(Hit.y, B1.y, Mathf.Pow(10, -4))) &&
                             (Hit.y < B2.y || MathUtils.IsEquals(Hit.y, B2.y, Mathf.Pow(10, -4)))) return true;

            if (Axis == 2 && (Hit.z > B1.z || MathUtils.IsEquals(Hit.z, B1.z, Mathf.Pow(10, -4))) &&
                             (Hit.z < B2.z || MathUtils.IsEquals(Hit.z, B2.z, Mathf.Pow(10, -4))) &&
                             (Hit.x > B1.x || MathUtils.IsEquals(Hit.x, B1.x, Mathf.Pow(10, -4))) &&
                             (Hit.x < B2.x || MathUtils.IsEquals(Hit.x, B2.x, Mathf.Pow(10, -4)))) return true;

            if (Axis == 3 && (Hit.x > B1.x || MathUtils.IsEquals(Hit.x, B1.x, Mathf.Pow(10, -4))) &&
                             (Hit.x < B2.x || MathUtils.IsEquals(Hit.x, B2.x, Mathf.Pow(10, -4))) &&
                             (Hit.y > B1.y || MathUtils.IsEquals(Hit.y, B1.y, Mathf.Pow(10, -4))) &&
                             (Hit.y < B2.y || MathUtils.IsEquals(Hit.y, B2.y, Mathf.Pow(10, -4)))) return true;
            return false;
        }
